interface Ways {
  spot: number[]; //path of spot each dice start at 1
  dice: number[]; // face of dice 1d6
}

export interface Boards {
  ladders: [number, number][]; //spot to climp up
  snakes: [number, number][]; //spot to climp down
}
...
function checkClimb(spot: number, checkPath: Boards): number {
...
function getNextSpot(
  startSpot: Ways[], // array of current path
  checkPath: Boards, // for check climp
  minThrows: number = 100 //filter throwing over minimum that we won't interest because we are finding minimum way of throwing to get spot 100
): Ways[] {
  const newWays: Ways[] = [];
  for (const way of startSpot) {
    if (way.dice.length >= minThrows && way.spot[way.spot.length - 1] !== 100) {
      ...
      for (let dice = 1; dice < 7; dice++) {
        const currentSpot: number = way.spot[way.spot.length - 1];
        let nextSpot: number = currentSpot + dice;
        if (nextSpot > 100) {
          nextSpot = 100 - (nextSpot - 100);
        }
        const updatedSpot: number = checkClimb(nextSpot, checkPath);
...
function quickestPath(board: Boards): number[][] {
  let startSpot: Ways[] = [{ spot: [1], dice: [] }];
  let finalPath: Ways[] = [];

  let minThrows: number = 100;
  while (true) {
    let nextWay: Ways[] = getNextSpot(startSpot, board, minThrows);
    ...
    for (const way of nextWay) {
      ...
      if (
        way.spot[way.spot.length - 1] === 100 &&
